...
/**
 * @title GemBotsNFA v5 — Non-Fungible Agents
 ...
 *      v5: Added batchSetTokenURI, batchMintGenesis, contractURI, tier-based mint pricing.
 */
contract GemBotsNFAv5 is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard, Pausable {
    ...
    /// @notice Genesis minting limit and counter
    uint256 public constant GENESIS_LIMIT = 100;
    uint256 public genesisCount;

    /// @notice Mint fees per tier (in wei)
    mapping(Tier => uint256) public tierMintFees;

    /// @notice Contract-level metadata URI (for OpenSea / marketplaces)
    ...
    function mint(
        ...
        // Enforce tier-based pricing (owner exempt for admin mints)
        if (msg.sender != owner()) {
            uint256 fee = tierMintFees[requestedTier];
            require(msg.value >= fee, "Insufficient mint fee");
            require(
                requestedTier == Tier.Bronze ||
                requestedTier == Tier.Silver ||
                requestedTier == Tier.Gold,
                "Can only mint Bronze/Silver/Gold"
            );
        }
    ...
    // ──────────────────────────────────────────────
    //  1b. Batch Mint Genesis (owner-only, free)
    // ──────────────────────────────────────────────

    /**
     * @notice Batch mint Genesis NFAs (owner-only, no fee).
     * @param count Number of NFAs to mint in this batch.
     */
    function batchMintGenesis(uint256 count) external onlyOwner whenNotPaused {
        require(genesisCount + count <= GENESIS_LIMIT, "Exceeds genesis limit");
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            emit NFAMinted(tokenId, 0, msg.sender, bytes32(0));
        }
    }

    // ──────────────────────────────────────────────
    //  1c. Batch Set Token URIs (owner-only)
    // ──────────────────────────────────────────────

    /**
     * @notice Batch-update tokenURIs for multiple NFAs.
